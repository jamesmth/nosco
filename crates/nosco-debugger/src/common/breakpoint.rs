...
impl BreakpointManager {
    ...
    pub fn add_breakpoint_or_increment_usage(
        &mut self,
        process_id: u64,
        addr: u64,
    ) -> sys::Result<Breakpoint> {
        let cx = match self.bks.entry(addr) {
            ...
            Entry::Vacant(v) => {
                ...
                sys::mem::read_process_memory(process_id, addr, &mut orig_opcodes)?;
        ...
        bk.enable(process_id)?;

        Ok(bk)
    }

    pub fn remove_breakpoint_or_decrement_usage(&mut self, process_id: u64, addr: u64) {
        ...
        if bk.get().ref_count == 0 {
            ...
            let _ = sys::mem::write_process_memory(process_id, addr, &bk.orig_opcodes);
        }
    }
}
...
impl Breakpoint {
    ...
    pub fn enable(&self, process_id: u64) -> sys::Result<()> {
        if !self.enabled() && !self.deleted() {
            sys::mem::write_process_memory(process_id, self.addr, &TRAP_OPCODES)?;
    ...
    pub fn disable(&self, process_id: u64) -> sys::Result<()> {
        if self.enabled() & !self.deleted() {
            sys::mem::write_process_memory(process_id, self.addr, &self.orig_opcodes)?;
